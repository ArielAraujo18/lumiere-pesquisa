...
export default function NewMemberPage() {
  ...
  const [member, setMember] = useState<MemberPayload>(initialMember);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Não foi possível salvar o membro.");
      }
  ...
  return (
    <main className="min-h-screen bg-[#f7f9f7] text-[#071a2b]">
      <header className="flex min-h-[68px] items-center gap-4 border-b border-black/5 bg-white px-5 sm:px-8">
        ...
        <div>
          <h1 className="text-lg font-semibold">Novo membro</h1>
      ...
      <form onSubmit={handleSubmit} className="p-5 sm:p-8">
        ...
        <MemberForm value={member} onChange={setMember} disabled={saving} />
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            disabled={saving}
            className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#27877d] px-5 text-sm font-medium text-white hover:bg-[#21766d] disabled:cursor-not-allowed disabled:opacity-60"
          >
